...
if display_flag==1
    %figure, title(['Similarity Measure : ' num2str(S2) num2str(S3)]);
    ...
    for  i = 1 : I
      ...
      %      Dist(i,27) = sqrt((Dist(i,5)-Dist(1,5))^2+(Dist(i,6)-Dist(1,6))^2);
      %      Dist(i,28) = sqrt((Dist(i,9)-Dist(1,9))^2+(Dist(i,10)-Dist(1,10))^2);
      %      Dist(i,29) = sqrt((Dist(i,13)-Dist(1,13))^2+(Dist(i,14)-Dist(1,14))^2);
      %      Dist(i,30) = sqrt((Dist(i,17)-Dist(1,17))^2+(Dist(i,18)-Dist(1,18))^2);
        end
        %{
        for j = 1 : count1
            if T12(j,5) == Dist_index(i,1)
                Dist(i,1) = T12(j,1);
                Dist(i,2) = T12(j,2);
                break;
            end
        ...
        %{
        Temp_x = (Dist(i,1)+Dist(i,5)+Dist(i,9)+Dist(i,13)+Dist(i,17))/3;
        Temp_y = (Dist(i,2)+Dist(i,6)+Dist(i,10)+Dist(i,14)+Dist(i,18))/3;
        
        Temp_dx = Temp_x - Dist(i,1);
        Temp_dy = Temp_x - Dist(i,2);
        Dist(i,3) = Dist(i,3) + Temp_dx;
        Dist(i,4) = Dist(i,4) + Temp_dy;
        
        Temp_dx = Temp_x - Dist(i,5);
        Temp_dy = Temp_x - Dist(i,6);
        Dist(i,7) = Dist(i,7) + Temp_dx;
        Dist(i,8) = Dist(i,8) + Temp_dy;
        
        Temp_dx = Temp_x - Dist(i,9);
        Temp_dy = Temp_x - Dist(i,10);
        Dist(i,11) = Dist(i,11) + Temp_dx;
        Dist(i,12) = Dist(i,12) + Temp_dy;
        
        Temp_dx = Temp_x - Dist(i,13);
        Temp_dy = Temp_x - Dist(i,14);
        Dist(i,15) = Dist(i,15) + Temp_dx;
        Dist(i,16) = Dist(i,16) + Temp_dy;
        
        Temp_dx = Temp_x - Dist(i,17);
        Temp_dy = Temp_x - Dist(i,18);
        Dist(i,19) = Dist(i,19) + Temp_dx;
        Dist(i,20) = Dist(i,20) + Temp_dy;
        
        Dist(i,21) = sqrt(Dist(i,3)^2+Dist(i,4)^2);
        Dist(i,22) = sqrt(Dist(i,7)^2+Dist(i,8)^2);
        Dist(i,23) = sqrt(Dist(i,11)^2+Dist(i,12)^2);
        Dist(i,24) = sqrt(Dist(i,15)^2+Dist(i,16)^2);
        Dist(i,25) = sqrt(Dist(i,19)^2+Dist(i,20)^2);
        %}
    end
